...
const files=walk(root),pages=files.filter(f=>f.endsWith('.html')),indexable=pages.filter(f=>path.basename(f)!=='404.html'&&!/<meta name="robots" content="noindex(?:,nofollow)?">/i.test(fs.readFileSync(f,'utf8')));
...
for(const file of pages){
  ...
  if(!html.includes('href="#contenido"')&&!/<meta name="robots" content="noindex(?:,nofollow)?">/i.test(html))issues.push(['skip-link',rel]);
  ...
  if(path.basename(file)==='404.html'||/<meta name="robots" content="noindex(?:,nofollow)?">/i.test(html))continue;
...
const forbidden=/localhost|file:\/\/|C:\\Users\\|href="#"|#proximamente|api[_-]?key\s*=|password\s*=|token\s*=/i;
for(const file of files.filter(f=>f.endsWith('.html')||f.endsWith('.css')||(f.endsWith('.js')&&!f.endsWith('.mjs')&&!f.includes(`${path.sep}vendor${path.sep}`)))){const text=fs.readFileSync(file,'utf8');if(forbidden.test(text)||/\bTODO\b/.test(text))issues.push(['forbidden-reference',path.relative(root,file)])}
